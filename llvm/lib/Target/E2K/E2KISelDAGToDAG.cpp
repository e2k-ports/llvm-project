...
public:
  explicit E2KDAGToDAGISel(E2KTargetMachine &tm) : SelectionDAGISel(tm) {}
...
bool E2KDAGToDAGISel::tryInlineAsm(SDNode *N){
  ...
  for(unsigned i = 0, e = N->getGluedNode() ? NumOps - 1 : NumOps; i < e; ++i) {
    ...
    if ((!IsTiedToChangedOp && (!HasRC || RC != E2K::IntRegsRegClassID))
        || NumRegs != 2)
      continue;
    ...
    if (Kind == InlineAsm::Kind_RegDef ||
      ...
      Register GPVR = MRI.createVirtualRegister(&E2K::IntPairRegClass);
      ...
      SDValue Sub0 = CurDAG->getTargetExtractSubreg(E2K::sub_even, dl, MVT::i32,
                                                    RegCopy);
      SDValue Sub1 = CurDAG->getTargetExtractSubreg(E2K::sub_odd, dl, MVT::i32,
                                                    RegCopy);
      ...
      SDValue Pair = SDValue(
          CurDAG->getMachineNode(
              ...
              {
                  CurDAG->getTargetConstant(E2K::IntPairRegClassID, dl,
                                            MVT::i32),
                  T0,
                  CurDAG->getTargetConstant(E2K::sub_even, dl, MVT::i32),
                  T1,
                  CurDAG->getTargetConstant(E2K::sub_odd, dl, MVT::i32),
              }),
          0);
    ...
    if(PairedReg.getNode()) {
      ...
      if (IsTiedToChangedOp)
        ...
        Flag = InlineAsm::getFlagWordForRegClass(Flag, E2K::IntPairRegClassID);
...
void E2KDAGToDAGISel::Select(SDNode *N) {
  ...
  case ISD::UDIV: {
    ...
    if (N->getOpcode() == ISD::SDIV) {
      TopPart = SDValue(CurDAG->getMachineNode(E2K::SRAri, dl, MVT::i32, DivLHS,
                                   CurDAG->getTargetConstant(31, dl, MVT::i32)),
                        0);
    } else {
      TopPart = CurDAG->getRegister(E2K::G0, MVT::i32);
    }
    TopPart = CurDAG->getCopyToReg(CurDAG->getEntryNode(), dl, E2K::Y, TopPart,
                                   SDValue())
                  .getValue(1);

    // FIXME: Handle div by immediate.
    unsigned Opcode = N->getOpcode() == ISD::SDIV ? E2K::SDIVrr : E2K::UDIVrr;
